...
if( !function_exists('estate_listing_interno') ):
function estate_listing_interno($post_id){
    
    $property_dueno      = esc_html(get_post_meta($post_id, 'property_dueno', true) );
    $property_telefonos  = esc_html(get_post_meta($post_id, 'property_telefonos', true) );
    $property_mail       = esc_html(get_post_meta($post_id, 'property_mail', true) );
    $property_contacto   = esc_html(get_post_meta($post_id, 'property_contacto', true) );
    $property_notas         = esc_html(get_post_meta($post_id, 'property_notas', true) );
    
    $return_string='';
    
    if ($property_dueno != ''){
        $return_string.='<div class="listing_detail col-md-4"><strong>'.__('Dueño').':</strong> ' . $property_dueno . '</div>'; 
    }
    if ($property_telefonos != ''){
        $return_string.= '<div class="listing_detail col-md-4"><strong>'.__('Teléfonos').':</strong> ' . $property_telefonos . '</div>'; 
    }
    if ($property_mail != ''){
        $return_string.= '<div class="listing_detail col-md-4"><strong>'.__('Mail').':</strong> ' . $property_mail . '</div>';
    }  
    if ($property_contacto != '') {
        $return_string.= '<div class="listing_detail col-md-4"><strong>'.__('Contacto Inicial').':</strong> ' . $property_contacto . '</div>'; 
    } 
    if ($property_notas != '') {
        $return_string.= '<div class="listing_detail col-md-4"><strong>'.__('Notas Adicionales').':</strong> ' . $property_notas . '</div>'; 
    } 
